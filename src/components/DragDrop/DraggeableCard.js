...
const DraggeableCard = props => {
    const {member,positionActive, dispatchSelectedMechanic, stateMechanicSelected} = props;
    ...
    return(
        <div id={member.id} className="card" draggable onDragStart={startDrag} onDragOver={ondragOver} onClick={selectedByClick}>
            {props.children}
        </div>
    );
}
