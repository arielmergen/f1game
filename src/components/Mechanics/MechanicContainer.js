...
const getMechanic = (name) =>
    WOMEN_NAMES.indexOf(name) !== -1 ? mechanic3Img : mechanic1Img;
    ...
    return (
        <div className="card-group">
            {team.members.map((member) => (
                ...
                >
                    <h6 className="card-header text-center text-white bg-danger">
                        {member.name}
                    </h6>
                    <img
                        src={getMechanic(member.name)}
                        className="card-img-top card-mechanic"
                        alt={member.name}
                        draggable="false"
                    />
