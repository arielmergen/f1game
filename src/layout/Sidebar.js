...
const RenderIfNeeded = ({display,teamname,itemDropped}) => {
    if(display){
        ...
        return (<h5 className="text-center text-uppercase text-secondary mb-2">
...
const Sidebar = (props) => {
    ...
    <div className="col-lg-3">
        <h4 className="text-center text-uppercase text-secondary mb-2">
            Team: {getUserState.name ? getUserState.name : 'Set your team name'} 
        </h4>
        <RenderIfNeeded display={getState.display} teamname={getUserState.name} itemDropped={getState.itemDropped} />
